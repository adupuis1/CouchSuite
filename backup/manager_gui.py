...
class ServerManagerApp:
    ...
    def _build_ui(self) -> None:
        ...
        if self.splash_image is not None:
            splash_frame = ttk.Frame(outer, style="Background.TFrame")
            splash_frame.pack(fill=tk.X, pady=(0, 12))
            splash_label = tk.Label(splash_frame, image=self.splash_image, bg=PALETTE["background"], borderwidth=0)
            splash_label.pack(anchor=tk.W)
